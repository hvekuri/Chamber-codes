...
def read_rpi(date):
    ...
    rpi = rpi[1:]
    rpi["TIME"] = pd.to_datetime(rpi["TIME"], format="%Y-%m-%d %H:%M:%S")
...
def plot_meas(
    ...
    ax.scatter(
        orig_licor.index.time,
        drift_a * orig_licor.co2_avg + drift_b,
        color="black",
        label="co2",
        s=3,
    )
    ...
    ax.set_title(
        ...
        + str(round(nrmse, 2))
        + " PAR sd: "
        + str(round(np.std(cur_rpi["PAR(umol/m2/s)"]), 2))
        + " Slope: "
        + str(round(slope, 2)),
        fontsize=6,
    )
    ...
    ax2.plot(orig_rpi.index.time, orig_rpi["PAR(umol/m2/s)"], c="darkblue", label="PAR")
...
def fit_LR(lr, collar, experiment, date, plotting):
    ...
    params = Parameters()
    params.add_many(
        ("alpha", -0.001, True, config.alpha_bounds[0], config.alpha_bounds[1]),
        ("GPmax", -1, True, config.GPmax_bounds[0], config.GPmax_bounds[1]),
    )
...
def plot_LR(result, alpha, GPmax, lr, collar, date):
    ...
    if result.covar is not None:
        ...
        ax.fill_between(
            xp, GPP_fit - GPP_unc, GPP_fit + GPP_unc, alpha=0.5, color="grey"
        )
    ...
    # plt.show()
    fig.savefig(
        config.results_path + config.experiment + "_LR_" + date + "_" + collar + ".png"
    )
    plt.close()


def add_GPP_in_diff_light_levels(result, extra_light_levels, idx):
    for par in config.GPP_light_levels:
        gpp = result.eval(PAR=par)
        if result.covar is not None:
            gpp_unc = result.eval_uncertainty(sigma=1.96, PAR=par)[0]
        else:
            gpp_unc = np.nan

        extra_light_levels.loc[idx, f"GP{par} [mg CO2 m-2 s-1]"] = gpp
        extra_light_levels.loc[idx, f"GP{par}_95perc_CI"] = gpp_unc


def main():
    # Read metadata
    metadata = pd.read_csv(config.metadata_path, delimiter=";")
    metadata.Date = pd.to_datetime(metadata.Date, format="%d.%m.%Y").dt.date
    cols = ["s1", "e1", "s2", "e2", "s3", "e3", "s4", "e4", "s5", "e5"]
    for col in cols:
        metadata[col] = pd.to_datetime(metadata[col], format="%H:%M:%S")
    starts = cols[::2]
    ...
    extra_light_levels = pd.DataFrame()
    ...
    for date in metadata.Date.unique():
        ...
        for idx, row in cur_metadata.iterrows():
            ...
            for i in range(len(starts)):
                ...
                slope, nrmse, rmse, co2_hat, cur_licor, cur_rpi, tang, meas_ok = (
                    find_good_meas(cur_licor, cur_rpi, drift_a, drift_b)
                )
                ...
                if np.isnan(temp):
                    ...
                    if np.isnan(temp):
                        print(
                            "Temperature missing, fill in metadata. Date: "
                            + str(date)
                            + " Collar: "
                            + str(row.Collar)
                        )
                ...
                if np.isnan(pres):
                    ...
                    if np.isnan(pres):
                        print(
                            "Air pressure missing, fill in metadata. Date: "
                            + str(date)
                            + " Collar: "
                            + str(row.Collar)
                        )
                ...
                if np.isnan(par):
                    ...
                    if np.isnan(par):
                        print(
                            "PAR missing, fill in metadata. Date: "
                            + str(date)
                            + " Collar: "
                            + str(row.Collar)
                        )
                ...
                sysvol = (
                    config.collar_area * (float(row["Collar_height (m)"]) - config.gap)
                    + row["Chamber_volume (m3)"]
                )

                # Calculate flux
                flux = calc_flux(slope, pres, sysvol, temp, config.collar_area)
            ...
            if len(lr) > 3:
                ...
                if resp is not None:
                    ...
                    alpha, alpha_se, GPmax, GPmax_se, GP1200, GP1200unc, result = (
                        fit_LR(
                            lr,
                            str(row.Collar),
                            config.experiment,
                ...
                add_GPP_in_diff_light_levels(result, extra_light_levels, idx)

                # Plot light response
                if config.plotting:
                    plot_LR(result, alpha, GPmax, lr, str(row.Collar), str(date))
    lightresponse_results = lightresponse_results.join(extra_light_levels)
    ...
    all_fluxes.to_csv(
        config.results_path + "Fluxes_" + config.experiment + ".csv", index=False
    )
